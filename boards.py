...
# ========== 异构系统板级配置 ==========
class AX7020Heterogeneous(Board):
    """AX7020 异构系统配置 - Linux SMP + 支持小核通信"""
    def __init__(self):
        from litex_boards.targets import alinx_ax7020
        Board.__init__(self, alinx_ax7020.BaseSoC, soc_capabilities={
            "leds",
    ...
    soc_kwargs = {
        "cpu_type"          : "vexriscv_smp",  # 使用SMP版本（带缓存）
        "cpu_count"         : 4,               # 2个核心
        "sys_clk_freq"      : 100e6,           # 100MHz
        "l2_size"           : 2048,            # 2KB L2缓存
        "with_heterogeneous": True,            # 启用异构支持
    }
